...
public newUserOriginalData = [];
  constructor( private afs: AngularFirestore) {
    this.getUserMarkers().then(data => {
      this.userData = data.sort((a,b) => (a.number > b.number) ? 1 : ((b.number > a.number) ? -1 : 0));
      this.userOriginalData = this.userOriginalData.sort((a,b) => (a.number > b.number) ? 1 : ((b.number > a.number) ? -1 : 0));
    });
    this.getNewUserMarkers().then(data => {
      this.newUserData = data.sort((a,b) => (a.number > b.number) ? 1 : ((b.number > a.number) ? -1 : 0));
      this.newUserOriginalData = this.newUserOriginalData.sort((a,b) => (a.number > b.number) ? 1 : ((b.number > a.number) ? -1 : 0));
    });
   }
   async getUserMarkers() {
    var markers = [];
    if(!this.userData) {
    await firebase.firestore().collection(this.userPathBase).get()
      .then(querySnapshot => {
  ...
  async getNewUserMarkers() {
    var markers = [];
    if(!this.newUserData) {
    await firebase.firestore().collection(this.newUserPathBase).get()
      .then(querySnapshot => {
        querySnapshot.docs.forEach(doc => {
