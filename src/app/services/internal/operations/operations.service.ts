        ...
        .then((querySnapshot) => {
          querySnapshot.docs.forEach((doc) => {
            if (doc.data().number.slice(2, 6) > this.highestEmployeeNumber) {
              this.highestEmployeeNumber = parseInt(
                doc.data().number.slice(2, 6)
              );
            }
            this.originalEmployeeData.push(doc.data());
            markers.push(doc.data());
            markers[markers.length - 1].dateOfBirthFormatted =
              this.dateFormatting(doc.data().dob);
        ...
        for (let i = 1; i < this.presentAttandanceData.length; i++) {
          ...
          for (let j = 0; j < this.presentAttandanceData[i].weeks.length; j++) {
            this.presentAttandanceData[i].compensationHrs =
              this.presentAttandanceData[i].compensationHrs +
              this.presentAttandanceData[i].weeks[j].compensationHrs;
            this.presentAttandanceData[i].weekHrs =
              this.presentAttandanceData[i].weekHrs +
              this.presentAttandanceData[i].weeks[j].weekHrs;
            this.presentAttandanceData[i].otHrs =
              this.presentAttandanceData[i].otHrs +
              this.presentAttandanceData[i].weeks[j].otHrs;
          }
          for (
            let j = 1;
            j < this.presentAttandanceData[i].dates.length - 1;
            j++
          ) {
          ...
          for (let k = 0; k < this.employeeData.length; k++) {
            if (
              this.employeeData[k].number ==
                this.presentAttandanceData[i].number &&
              !this.presentAttandanceData[i].salary
            ) {
              this.presentAttandanceData[i].salary =
                this.employeeData[k].salary.monthlySalary;
              this.presentAttandanceData[i].bonus =
                this.employeeData[k].salary.monthlyFixedBonus;
            }
          }
        }
      });
