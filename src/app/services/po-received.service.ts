...
export class PoReceivedService {
    ...
    if (allFiles) {
      for (let i = 0; i < allFiles.length; i++) {
        ...
        if (editFlag && po.files.customerDocuments.length) {
          po.files.customerDocuments[po.files.customerDocuments.length] = {
            ...poFile,
          };
        } else {
  ...
  async getNewPOs() {
    ...
    try {
      purchaseOrders = await this.afs
        .collection<PurchaseOrder>(this.pathBase, (query) =>
          query.where("status", "==", "New").orderBy("number", "asc").limit(20)
        )
        .valueChanges()
        .pipe(take(1))
        .toPromise();
    ...
    for (let i = 0; i < purchaseOrders.length; i++) {
      this.pos[i].issueDateFormatted = this.dateFormatting(
        purchaseOrders[i].issueDate
      );
      this.pos[i].deliveryDateFormatted = this.dateFormatting(
        purchaseOrders[i].deliveryDate
      );
      this.pos[i].flag = true;
      this.pos[i].totalPrice = 0;
      for (let j = 0; j < this.pos[i].itemPrice.length; j++) {
        this.pos[i].totalPrice =
          this.pos[i].totalPrice + this.pos[i].itemPrice[j].totalPrice;
      }
      this.pos[i].totalPriceGst =
        this.pos[i].totalPrice + this.pos[i].totalPrice * 0.18;
    }
  }
