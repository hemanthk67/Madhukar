...
export class NewPurchaseOrderReceivedComponent implements OnInit {
  ...
  submit() {
    if (this.editFlag) {
      this.po = this.poreceivedservice.uploadFile(
        this.editFileAdd,
        this.po,
        this.editFlag
      );
      for (let j = 0; j < this.poreceivedservice.originalPos.length; j++) {
        if (this.poreceivedservice.originalPos[j].number == this.po.number) {
          ...
          this.poreceivedservice.pos[j].issueDateFormatted =
            this.poreceivedservice.dateFormatting(this.po.issueDate);
          this.poreceivedservice.pos[j].deliveryDateFormatted =
            this.poreceivedservice.dateFormatting(this.po.deliveryDate);
          this.poreceivedservice.pos[j].flag = true;
          this.poreceivedservice.pos[j].totalPrice = 0;
          for (
            let k = 0;
            k < this.poreceivedservice.pos[j].itemPrice.length;
            k++
          ) {
            this.poreceivedservice.pos[j].totalPrice =
              this.poreceivedservice.pos[j].totalPrice +
              this.poreceivedservice.pos[j].itemPrice[k].totalPrice;
          }
          this.poreceivedservice.pos[j].totalPriceGst =
            this.poreceivedservice.pos[j].totalPrice +
            this.poreceivedservice.pos[j].totalPrice * 0.18;
        }
      }
    } else {
      this.po = this.poreceivedservice.uploadFile(
  ...
  addPrice(i) {
    if (this.po.itemPrice[i].unitPrice) {
      this.po.itemPrice[i].totalPrice =
        this.po.itemPrice[i].unitPrice * this.po.itemPrice[i].qty;
      this.po.itemPrice[i].totalWithGst =
        this.po.itemPrice[i].totalPrice * (1 + this.po.itemPrice[i].gst / 100);
    }
    this.total = 0;
    ...
    this.totalWithGst =
      this.total + this.total * (this.po.itemPrice[0].gst / 100);
  }
