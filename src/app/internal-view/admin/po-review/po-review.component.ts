...
export class PoReviewComponent implements OnInit {
  ...
  ngOnInit() {}
  read(index) {
    this.poReceivedService.pos[
      this.poReceivedService.pos.length - 1 - index
    ].flag =
      !this.poReceivedService.pos[this.poReceivedService.pos.length - 1 - index]
        .flag;
  ...
  approval(value, index) {
    this.poReceivedService.originalPos[
      this.poReceivedService.originalPos.length - index - 1
    ].status = value;
    this.poReceivedService.setPoData(
      this.poReceivedService.originalPos[
        this.poReceivedService.originalPos.length - index - 1
      ]
    );
    ...
    this.productionService.newWorkOrder(
      this.poReceivedService.originalPos[
        this.poReceivedService.originalPos.length - index - 1
      ]
    );
