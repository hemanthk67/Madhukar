...
export class PrepareOfferComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < this.infoService.pvtCustomerData.length; i++) {
      if (
        this.infoService.pvtCustomerData[i].fullName == this.enquiry.customer
      ) {
    ...
    if (this.enquiry.firm) {
      if (this.enquiry.firm == "THOTA COLDCEL PVT LTD") {
        ...
        this.offer.terms[8].content =
          "Bank Name: State Bank of India - Srila Park Pride Kukatpally Branch, Account No: 36730666393, IFSC:SBIN0018875 ";
      } else {
        this.pdfType = "thota";
      }
    } else {
  ...
  addCustomerReference() {
    if (this.newCustomerReference.name !== "") {
      ...
      this.infoService.updatePvtCustomerName(
        JSON.parse(JSON.stringify(this.newCustomerReference)),
        this.customerReferenceIndex
      );
  ...
  referenceChange() {
    for (let i = 0; i < this.customerReference.details.length; i++) {
      console.log(
        this.customerReference.details[i].name,
        this.offer.customer.refer
      );
  ...
  addItem() {
    var item = { description: "", unitPrice: 0.0, qty: 0.0, totalPrice: 0.0 };
    this.offer.itemsPrice.push(item);
  }
  removeItem(index) {
    this.offer.totalPrice =
      this.offer.totalPrice - this.offer.itemsPrice[index].totalPrice;
    this.offer.itemsPrice.splice(index, 1);
    this.subTotalPrice = this.offer.totalPrice + ".00/-";
    this.subTotalPriceWords =
      this.convertNumberToWords(this.offer.totalPrice) + "Only";
  }
  addTerm() {
    var term = { lable: "", content: "" };
  ...
  addPrice(index) {
    this.offer.totalPrice =
      this.offer.totalPrice - this.offer.itemsPrice[index].totalPrice;
    this.offer.itemsPrice[index].totalPrice =
      this.offer.itemsPrice[index].unitPrice *
        this.offer.itemsPrice[index].qty +
      0.0;
    this.offer.totalPrice =
      this.offer.totalPrice + this.offer.itemsPrice[index].totalPrice;
    this.subTotalPrice = this.offer.totalPrice + ".00/-";
  ...
  offerDataFormate() {
    ...
    this.documentName =
      this.enquiry.customer + " OFFER P - " + this.enquiry.number;
  }
  offerDataAssigning() {
    ...
    for (let i = 0; i < this.enquiry.items.length; i++) {
      if (i == this.enquiry.items.length - 1 && i !== 0) {
        this.offer.subject =
          this.offer.subject +
          "and " +
          this.enquiry.items[i].rating +
          "KVA-" +
          this.enquiry.items[i].classHv +
          "/" +
          this.enquiry.items[i].classLv +
          "KV.";
      } else {
        this.offer.subject =
          this.offer.subject +
    ...
    for (let i = 0; i < this.infoService.employeeData.length; i++) {
      if (this.enquiry.employee === this.infoService.employeeData[i].name) {
        ...
        this.offer.authorizedPhoneNumber =
          this.infoService.employeeData[i].phoneNumber;
