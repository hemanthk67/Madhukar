...
export class MarketingResultComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    for(let i =0; i< this.marketingService.enquiry.itemPrice.length; i++) {
      ...
      this.total = this.total + this.marketingService.enquiry.itemPrice[i].totalPrice;
      this.marketingService.enquiry.itemPrice[i].totalWithGst = this.marketingService.enquiry.itemPrice[i].totalPrice*( 1 + (this.marketingService.enquiry.itemPrice[i].gst/100));
    }
    this.totalWithGst = this.total + this.total*(this.marketingService.enquiry.itemPrice[0].gst/100);
  }
    ...
    this.resultStatus = status;
      ...
      addPrice(i) {
        if(this.marketingService.enquiry.itemPrice[i].unitPrice) {
          this.marketingService.enquiry.itemPrice[i].totalPrice = (this.marketingService.enquiry.itemPrice[i].unitPrice)*this.marketingService.enquiry.itemPrice[i].qty;
          this.marketingService.enquiry.itemPrice[i].totalWithGst = (this.marketingService.enquiry.itemPrice[i].totalPrice )*(1+(this.marketingService.enquiry.itemPrice[i].gst/100));
        }
        this.total =0;
        for(let j =0; j< this.marketingService.enquiry.itemPrice.length; j++) {
          this.total = this.total + this.marketingService.enquiry.itemPrice[j].totalPrice;
        }
        this.totalWithGst = this.total + this.total*(this.marketingService.enquiry.itemPrice[0].gst/100);
      }
      submit() {
