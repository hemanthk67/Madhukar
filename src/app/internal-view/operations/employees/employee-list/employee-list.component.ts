...
export class EmployeeListComponent implements OnInit {
  ...
  userStatus(status, index) {
    ...
    if (status == "add-back") {
      ...
      if (
        this.operations.employeeData[index].active ==
        this.operations.originalEmployeeData[index].active
      ) {
        this.changeFlag = false;
      }
  ...
  submit() {
    this.operations.originalEmployeeData[this.changeIndex].active =
      this.operations.employeeData[this.changeIndex].active;
    this.changeFlag = false;
    this.operations.changeEmployeeStatus(
      this.operations.originalEmployeeData[this.changeIndex],
      this.seperationFlag,
      this.changeIndex
    );
    this.seperationFlag = false;
  ...
  start() {
    if (this.startFlag) {
      for (let i = 0; i < this.operations.employeeData.length; i++) {
        this.imageService
          .getImageUrl(this.operations.employeeData[i].photoPath)
          .then((url) => {
  ...
  startSeperated() {
    if (this.startSeperatedFlag) {
      for (let i = 0; i < this.operations.SeperatedEmployeeData.length; i++) {
        this.imageService
          .getImageUrl(this.operations.SeperatedEmployeeData[i].photoPath)
          .then((url) => {
