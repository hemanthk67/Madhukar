...
export class EandwAttandanceComponent implements OnInit {
  ...
  weeks = [
    ...
    { number: 6, name: "Final", flag: false, dates: [], days: [] },
  ];
  ...
  newAttandanceData() {
    ...
    for (let i = 0; i < this.weeks.length; i++) {
      for (let j = 0; j < this.weeks[i].dates.length; j++) {
        this.weeks[i].days[j] = this.dayByDate(
          this.weeks[i].dates[j],
          this.weeks[i].number
        );
    ...
    for (let i = 0; i < this.operations.employeeData.length; i++) {
      ...
      this.attandancePerEmployee.number =
        this.operations.employeeData[i].number;
      this.attandancePerEmployee.type = this.operations.employeeData[i].type;
      this.attandancePerEmployee.weeks = [
        {
          weekHrs: 0,
          compensationHrs: 0,
          otHrs: 0,
        ...
        for (let j = 0; j < this.weeks[i].dates.length; j++) {
          ...
          if (this.weeks[i].days[j] !== null) {
            if (this.weeks[i].number == 1 && this.weeks[i].dates[j] == 20) {
              ...
              this.attandancePerEmployee.weeks[i].weekHrs =
                this.attandancePerEmployee.weeks[i].weekHrs + 0;
            } else if (
              this.weeks[i].number == 5 &&
              this.weeks[i].dates[j] == 21
            ) {
              dateProperties.date = this.weeks[i].dates[j];
              this.attandancePerEmployee.dates.push({ ...dateProperties });
              this.attandancePerEmployee.weeks[i].weekHrs =
              ...
              this.attandancePerEmployee.dates.push({ ...dateProperties });
              this.attandancePerEmployee.weeks[i].weekHrs =
                this.attandancePerEmployee.weeks[i].weekHrs + 8;
            }
          }
        }
      }
  ...
  dayByDate(date, weekNumber) {
    ...
    if (this.selectedMonth !== 0) {
      if (weekNumber == 5 && date == 21) {
        ...
        if (date > 20) {
          ...
          presentDate =
            this.selectedMonth + 1 + presentDate + this.selectedYear;
        }
      }
    } else {
      if (weekNumber == 5 && date == 21) {
    ...
    if (
      ...
      this.selectedMonth == new Date(presentDate).getMonth()
    ) {
      return null;
    }
  ...
  weekRole(type, index, weekIndex) {
    if (type == "H") {
      for (let i = 0; i < this.newAttandance.length; i++) {
        this.newAttandance[i].dates[index].holiday =
          !this.newAttandance[i].dates[index].holiday;
      ...
      for (let i = 0; i < this.newAttandance.length; i++) {
        ...
        this.newAttandance[i].dates[index].compensationHoliday =
          !this.newAttandance[i].dates[index].compensationHoliday;
        this.newAttandance[i].dates[index].weekOff = false;
      ...
      for (let i = 0; i < this.newAttandance.length; i++) {
        this.newAttandance[i].dates[index].holiday = false;
        this.newAttandance[i].dates[index].compensationHoliday = false;
        this.newAttandance[i].dates[index].weekOff =
          !this.newAttandance[i].dates[index].weekOff;
  ...
  dayRole(employeeIndex, type, index, weekIndex) {
    ...
    if (type == "L") {
      this.newAttandance[employeeIndex].dates[index].leave =
        !this.newAttandance[employeeIndex].dates[index].leave;
      this.newAttandance[employeeIndex].dates[index].absent = false;
      if (
        employeeIndex !== 0 &&
        (this.newAttandance[employeeIndex].dates[index].compensationHoliday ||
          (employeeIndex !== 0 &&
            this.newAttandance[employeeIndex].dates[index].weekOff) ||
          this.newAttandance[employeeIndex].dates[index].holiday)
      ) {
      ...
      this.newAttandance[employeeIndex].dates[index].absent =
        !this.newAttandance[employeeIndex].dates[index].absent;
  ...
  getTimeDifference(employeeIndex, weekIndex) {
    ...
    for (let j = 0; j < this.weeks[0].dates.length; j++) {
      if (this.weeks[0].days[j] !== null) {
        ...
        var timeIn =
          this.newAttandance[employeeIndex].dates[
            this.getAttandanceArrayIndex(
              this.weeks[0].dates[j],
              this.weeks[0].number
            )
          ].timeIn;
        var timeOut =
        ...
        if (
          ...
          if (
            ...
            hours = hours + 0.5 + 8;
            this.newAttandance[employeeIndex].weeks[weekIndex].compensationHrs =
              this.newAttandance[employeeIndex].weeks[weekIndex]
                .compensationHrs + 8;
          } else {
            hours = hours + 0.5;
          }
          ...
          if (
            ...
            this.newAttandance[employeeIndex].weeks[weekIndex].holidayHrs =
              this.newAttandance[employeeIndex].weeks[weekIndex].holidayHrs + 8;
            hours = hours + 0.5 + 8;
          } else {
            hours = hours + 0.5;
          }
          ...
          if (
            ...
            this.newAttandance[employeeIndex].weeks[weekIndex].weekOffHrs =
              this.newAttandance[employeeIndex].weeks[weekIndex].weekOffHrs + 8;
            hours = hours + 0.5 + 8;
          } else {
            hours = hours + 0.5;
          }
        ...
        if (hours + minutes / 60 > 8) {
          this.newAttandance[employeeIndex].weeks[weekIndex].otHrs =
            this.newAttandance[employeeIndex].weeks[weekIndex].otHrs +
            hours +
            minutes / 60 -
            8;
          this.newAttandance[employeeIndex].weeks[weekIndex].weekHrs =
            this.newAttandance[employeeIndex].weeks[weekIndex].weekHrs + 8;
        } else {
          this.newAttandance[employeeIndex].weeks[weekIndex].weekHrs =
            this.newAttandance[employeeIndex].weeks[weekIndex].weekHrs +
            hours +
  ...
  attandanceDateChange() {
    ...
    if (this.operations.presentAttandanceDate !== "") {
      this.startFlag = true;
      this.operations.getEmployeeAttandanceData(
  ...
  submit(value) {
    ...
    if (this.newAttandanceFlag) {
      ...
      this.operations.employeeAttandance(
        JSON.parse(JSON.stringify(this.newAttandance)),
        this.selectedDate
      );
