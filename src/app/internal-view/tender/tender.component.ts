...
export class TenderComponent implements OnInit {
  ...
  ngOnInit() {}
  read(index) {
    this.tenderService.data[this.tenderService.data.length - 1 - index].flag =
      !this.tenderService.data[this.tenderService.data.length - 1 - index].flag;
  ...
  approval(value, index) {
    this.tenderService.originalData[
      this.tenderService.originalData.length - index - 1
    ].status = value;
    this.tenderService.setTenderData(
      this.tenderService.originalData[
        this.tenderService.originalData.length - index - 1
      ]
    );
    this.tenderService.data[
      this.tenderService.originalData.length - index - 1
    ].status = value;
  }
  documentprep(index) {
