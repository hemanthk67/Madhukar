...
export class ResultsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    for (let i = 0; i < this.tenderService.tender.itemsPrice.length; i++) {
      ...
      data.frieghtCharges =
        this.tenderService.tender.itemsPrice[i].frieghtCharges;
      ...
      this.comparitiveItemsPrice[0].total =
        this.comparitiveItemsPrice[0].total + data.total;
      this.comparitiveItemsPrice[0].totalWithGst =
        this.comparitiveItemsPrice[0].totalWithGst + data.totalWithGst;
      items.push({ ...data });
    }
  ...
  removeCompetitor(index) {
    for (let x = 0; x < this.competitorDetailsRemove.length; x++) {
      if (
        this.competitorDetailsRemove[x].name ===
        this.comparitiveItemsPrice[index].name
      ) {
      ...
      if (this.newCompetitorDetails.loacationByState != "") {
        ...
        if (!this.addnewMakeFlag) {
          if (this.newCompetitorDetails.brand != "") {
            ...
            this.infoService.addCompetitorDetails(
              { ...this.newCompetitorDetails },
              { ...this.newMakeDetails },
              this.addnewMakeFlag
            );
  ...
  addPrice(i, j) {
    ...
    if (this.comparitiveItemsPrice[j].data[i].pricePerItem) {
      this.comparitiveItemsPrice[j].data[i].total =
        this.comparitiveItemsPrice[j].data[i].total +
        this.comparitiveItemsPrice[j].data[i].pricePerItem *
          this.comparitiveItemsPrice[j].data[i].quantity;
      this.comparitiveItemsPrice[j].data[i].totalWithGst =
        this.comparitiveItemsPrice[j].data[i].totalWithGst +
        this.comparitiveItemsPrice[j].data[i].pricePerItem *
          (1 + this.comparitiveItemsPrice[j].data[i].gst / 100) *
          this.comparitiveItemsPrice[j].data[i].quantity;
    }
    if (this.comparitiveItemsPrice[j].data[i].frieghtCharges) {
      this.comparitiveItemsPrice[j].data[i].total =
        this.comparitiveItemsPrice[j].data[i].total +
        this.comparitiveItemsPrice[j].data[i].frieghtCharges *
          this.comparitiveItemsPrice[j].data[i].quantity;
      this.comparitiveItemsPrice[j].data[i].totalWithGst =
        this.comparitiveItemsPrice[j].data[i].totalWithGst +
        this.comparitiveItemsPrice[j].data[i].frieghtCharges *
    ...
    if (this.comparitiveItemsPrice[j].data[i].otherCharges) {
      this.comparitiveItemsPrice[j].data[i].total =
        this.comparitiveItemsPrice[j].data[i].total +
        this.comparitiveItemsPrice[j].data[i].otherCharges *
          this.comparitiveItemsPrice[j].data[i].quantity;
      this.comparitiveItemsPrice[j].data[i].totalWithGst =
        this.comparitiveItemsPrice[j].data[i].totalWithGst +
        this.comparitiveItemsPrice[j].data[i].otherCharges *
    ...
    for (let k = 0; k < this.comparitiveItemsPrice[j].data.length; k++) {
      if (
        this.comparitiveItemsPrice[j].data[k].total &&
        this.comparitiveItemsPrice[j].data[k].totalWithGst
      ) {
        this.comparitiveItemsPrice[j].total =
          this.comparitiveItemsPrice[j].total +
          this.comparitiveItemsPrice[j].data[k].total;
        this.comparitiveItemsPrice[j].totalWithGst =
          this.comparitiveItemsPrice[j].totalWithGst +
          this.comparitiveItemsPrice[j].data[k].totalWithGst;
      }
    }
